...
import { resolveSecretsConfig } from './config.js';
...
export async function doctor() {
  ...
  const effective = cfgRes.ok ? resolveSecretsConfig(cfgRes.value) : null;

  process.stdout.write(`${pc.bold('claude-secrets doctor')}\n`);
  ...
  for (const p of paths) {
    ...
    for (const e of HOOK_EVENTS) {
      ...
      if (dup) process.stdout.write(pc.dim('    Tip: re-run setup/remove to clean duplicates; claude-secrets only manages its own handlers.') + '\n');
    }
  }
}
