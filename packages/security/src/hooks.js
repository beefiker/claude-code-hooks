...
export function buildManagedCommand({ eventName, mode }) {
  ...
  return `npx --yes @claude-code-hooks/security@latest run --event ${eventName} --mode ${safeMode} ${MANAGED_TOKEN}`;
}

export function applySecurityToSettings(settings, { enabledEvents, mode }) {
...
export function removeAllManagedSecurityHooks(settings) {
  return applySecurityToSettings(settings, { enabledEvents: [], mode: 'warn' });
}
